...
function dodelete(tc)
{
	if  (!confirm("Are you sure you want to delete tournament " + tc))
		return;
	document.location = "deletetourn?tcode=" + tc;
}
</script>
...
EOT;
		...
		foreach ($tlist as $tc)  {
			...
			print <<<EOT
<tr>
	<td><a href="clonetourn$url">$tc</a></td>
...
EOT;
	if ($tourn->is_over())
		print <<<EOT
	<td>entry</td>
	<td><a href="listentries$url">list</a></td>
	<td>update</td>
...
EOT;
		...
		print <<<EOT
	<td><a href="tentform$url">entry</a></td>
	<td><a href="listentries$url">list</a></td>
	<td><a href="updtourn$url">update</a></td>
	<td><a href="javascript:dodelete('$tc')">delete</a></td>
</tr>
...
EOT;
	...
	print <<<EOT
<p><a href="createtourn">Click here</a> to set up a new tournament. Click on a tournament code above
